...
<body>
	...
	<c:out value="${percent * 100 }"/> % <br>
	<!-- to_char() >> string  -->
	
	<hr>
	
	기본 데이트 출력 : <c:out value="${now }"/> <br>
	기본 포맷(date) : <fmt:formatDate type="date" value="${now }"/> <br>
	기본 포맷(time) : <fmt:formatDate type="time" value="${now }"/> <br>
	기본 포맷(both) : <fmt:formatDate type="both" value="${now }"/> <br>
